...
class SiriusScraper:
    ...
    def scrape_page(self, url, species):
        ...
        if skills:
            ...
            data['Size'] = size
            data['SkillsAndCharacter'] = ", ".join(skill_list)
        else:
            data['Size'] = ''
            data['SkillsAndCharacter'] = ''

        history = soup.find('div', class_='profile-history')
        if history:
            [h4.decompose() for h4 in history.find_all('h4')]
            data['MyStory'] = self.clean_text(history.get_text())
        else:
            data['MyStory'] = ''
                ...
                while True:
                    ...
                    for href in links:
                        ...
                        if full_url in existing_urls:
                            self.cursor.execute(
                                '''UPDATE animals SET Name=?, Age=?, Gender=?, Size=?, SkillsAndCharacter=?, MyStory=?, PhotoURL=?, Species=?
                                WHERE ProfileURL=?''',
                                (data['Name'], data['Age'], data['Gender'], data['Size'], data['SkillsAndCharacter'],
                                 data['MyStory'], data['PhotoURL'], data['Species'], full_url))
                            logging.info(f"Updated entry: {full_url}")
                        else:
                            self.cursor.execute(
                                '''INSERT INTO animals (ProfileURL, Name, Age, Gender, Size, SkillsAndCharacter, MyStory, PhotoURL, Species)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                                (data['ProfileURL'], data['Name'], data['Age'], data['Gender'], data['Size'],
                                 data['SkillsAndCharacter'], data['MyStory'], data['PhotoURL'], data['Species']))
